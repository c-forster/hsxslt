...
<xsl:stylesheet xmlns:tei="http://www.tei-c.org/ns/1.0"
  ...
  <xsl:template match="tei:text[@xml:id='hs']//tei:lg[@type='poem'] |
    ...
    <!-- For Poems. -->
    <xsl:result-document method="text" href="text/{$poemid}.txt"><xsl:apply-templates /></xsl:result-document>
  </xsl:template>
  ...
  <xsl:template match="tei:div[@type='introductory-prose'] | tei:text[@type='review'] | tei:text[@type='supplementary']">
    ...
    <xsl:result-document method="text" href="text/{$id}"><xsl:apply-templates /></xsl:result-document>
  </xsl:template>
  ...
  <xsl:template match="tei:lg[@type='poem']"><xsl:choose>
    <xsl:when test="not(tei:lg)"><xsl:text>
      
      </xsl:text><xsl:call-template name="stanza" /></xsl:when>
      <xsl:otherwise><xsl:for-each select="tei:lg"><xsl:call-template name="stanza" /></xsl:for-each></xsl:otherwise>
      </xsl:choose>
  </xsl:template>
  
  <xsl:template name="stanza"><xsl:apply-templates select="tei:l" /><xsl:text>
...
<!--  <xsl:template match="tei:lg/text()"><xsl:value-of select="." /></xsl:template> -->
